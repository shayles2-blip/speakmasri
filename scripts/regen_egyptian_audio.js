...
const AUDIO_SAMPLE_RATE_HERTZ = 24000;
...
function parseArgs(argv) {
  const options = { limit: Infinity, only: null };
...
async function synthesize(entry, token) {
  const phonemeText = entry.ar
    .replace(/[\/—]/g, ' ')
    .replace(/[،,.؟!\[\]…«»]/g, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
  const ssml = `<speak><phoneme alphabet="ipa" ph="${escapeXmlAttribute(entry.ipa)}">${escapeXmlText(phonemeText)}</phoneme></speak>`;
  ...
  if (!data.audioContent) throw new Error('API response did not contain audioContent');
...
async function main() {
  ...
  console.log(`Regenerating ${jobs.length} file(s) with ${VOICE} and Egyptian IPA...`);
  let token = getToken();
  ...
  for (const [jobIndex, entry] of jobs.entries()) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      try {
        const audio = await synthesize(entry, token);
        ...
        if (authError) {
          try {
            ...
            lastError = new Error(`token refresh failed: ${tokenError.message}`);
            break;
          }
        }
